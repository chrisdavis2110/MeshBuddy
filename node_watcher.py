...
class NodeWatcher:
    ...
    def __init__(self, nodes_file: str, reserved_nodes_file: str, removed_nodes_file: str, category_id: Optional[int] = None, owners_file: Optional[str] = None, category_name: Optional[str] = None):
        ...
        # Extract category name from nodes_file if not provided
        if category_name is None:
            # Extract from nodes_file (e.g., "nodes_socal.json" -> "socal")
        ...
        # Construct offReserved filename based on category_name
        if category_name:
            self.off_reserved_nodes_file = f"offReserved_{category_name}.json"
        else:
    ...
    def load_off_reserved_nodes(self) -> Optional[Dict]:
        """Load offReserved_category.json and return the data"""
        # Retry logic to handle race conditions when file is being written
        max_retries = 3
        retry_delay = 0.5  # seconds
    ...
    def save_off_reserved_nodes(self, data: Dict):
        """Save offReserved_category.json"""
        try:
            data["timestamp"] = datetime.now().isoformat()
...
def main():
    ...
    for category_id, section in category_sections:
        ...
        watcher = NodeWatcher(nodes_file, reserved_nodes_file, removed_nodes_file, category_id, owners_file, category_name)
