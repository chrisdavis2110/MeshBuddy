...
class MQTTSubscriber:
    def __init__(self, config_file="config.ini"):
        ...
        # Initialize a single processor that outputs to nodes.json
        self.processor = NodeDataProcessor(log_file=None, api_url=self.api_url, output_file="nodes.json")
        # Load existing nodes to preserve first_seen
        self.processor._load_existing_nodes()
        # Create empty file if it doesn't exist
        self.processor.create_empty_nodes_file()
    ...
    def process_packet_to_nodes(self, topic, data, timestamp, region):
        """Process packet directly to nodes.json using NodeDataProcessor"""
        try:
            ...
            # Use the single processor for all regions
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if isinstance(data, dict) and 'data' in data:
                    ...
                    # Update processor with API data
                    self.processor._update_from_api_data(nodes)

                    # Save nodes.json
                    try:
                        self.processor.save_nodes_json()
                    except Exception as e:
                        self.logger.error(f"Error saving nodes.json: {e}")
    ...
    def _save_all_nodes(self):
        """Save nodes.json file"""
        self.logger.info("Saving nodes.json...")
        try:
            self.processor.save_nodes_json()
        except Exception as e:
            self.logger.error(f"Error saving nodes.json: {e}")
