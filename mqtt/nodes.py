...
class NodeDataProcessor:
    ...
    def _update_from_api_data(self, api_nodes_list):
        """Update nodes directly from API data (used when MQTT is not available)"""
        try:
            for api_node in api_nodes_list:
                public_key = api_node.get('public_key', '')
                if not public_key:
                    continue

                # Convert API node format to our node format
                # API nodes may have different field names, so we need to map them
                node_data = {
                ...
                # Handle timestamps
                if 'last_seen' in api_node:
                    last_seen = api_node['last_seen']
                    if isinstance(last_seen, str):
                        try:
                            ...
                            node_data['timestamp'] = int(dt.timestamp())
                            node_data['last_seen'] = api_node['last_seen']
                        except (ValueError, AttributeError):
                            node_data['last_seen'] = last_seen
                            node_data['timestamp'] = int(datetime.now().timestamp())
                    else:
                        node_data['timestamp'] = last_seen if isinstance(last_seen, (int, float)) else int(datetime.now().timestamp())
                        node_data['last_seen'] = datetime.fromtimestamp(node_data['timestamp']).isoformat() + 'Z'
                else:
                    node_data['timestamp'] = int(datetime.now().timestamp())
                    node_data['last_seen'] = datetime.now().isoformat() + 'Z'

                # Handle first_seen
                if 'first_seen' in api_node:
                    node_data['first_seen'] = api_node['first_seen']
                elif public_key in self.nodes and 'first_seen' in self.nodes[public_key]:
                    node_data['first_seen'] = self.nodes[public_key]['first_seen']
                else:
                    node_data['first_seen'] = node_data['last_seen']

                # Update or add node (keep newer timestamp if both exist)
                if public_key in self.nodes:
                    existing_ts = self.nodes[public_key].get('timestamp', 0)
                    new_ts = node_data.get('timestamp', 0)
                    if new_ts > existing_ts:
                        # Preserve first_seen from existing node if it's older
                        if 'first_seen' in self.nodes[public_key]:
                            existing_first = self.nodes[public_key]['first_seen']
                            try:
                                existing_first_ts = datetime.fromisoformat(existing_first.rstrip('Z')).timestamp()
                                new_first_ts = datetime.fromisoformat(node_data['first_seen'].rstrip('Z')).timestamp()
                                if existing_first_ts < new_first_ts:
                                    node_data['first_seen'] = existing_first
                            except Exception:
                                pass
            ...
            print(f"Error updating from API data: {e}")
    ...
    def decode_and_store(self, hex_string, log_timestamp):
        ...
        try:
            ...
            if not signature_valid:
                print("Warning: Packet signature verification failed, skipping node")
            ...
            if public_key in self.nodes:
                ...
                if effective_timestamp <= existing_timestamp:
                    ...
                    return
                # Skip if we already have a newer entry for this node
                # (timestamp comparison happens above)
    ...
    def create_empty_nodes_file(self, output_file=None):
        """Create an empty node file if it doesn't exist"""
        if output_file is None:
            output_file = self.output_file
        ...
        if not output_path.exists():
            # Create empty node file structure
            data = {
                "timestamp": datetime.now().isoformat() + 'Z',
                "data": []
            }
            with open(output_file, 'w') as f:
                json.dump(data, f, indent=2)
            print(f"Created empty node file: {output_file}")
